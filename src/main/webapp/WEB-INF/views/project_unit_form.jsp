...
<body>
	...
	<form:form method="post" action="add_unit/${myRef}" commandName="unit">
		...
		<table>
			...
			Разработчик
				...
				<td>
				<form:select path="id_user_creator" size="1" >
						<c:if test="${!empty wokers}">
						<c:forEach items="${wokers}" var="woker">
						<option value="${woker.id }">${woker.name}</option>
						</c:forEach>
						</c:if>
				</form:select>
				</td>
			...
			Норм.контроллер
				...
				<td>	<form:select path="id_user_norm" size="1" >
						<c:if test="${!empty wokers}">
						<c:forEach items="${wokers}" var="woker">
						<option value="${woker.id }">${woker.name}</option>
						</c:forEach>
			...
			Дата контроля
				...
				<td><form:input path="date_control"
						value="${unit.date_control}" /></td>
			</tr>
			<tr>
		  ...
		  Еденицы измерения
				...
				<td><form:input path="mesure_units"
						value="${unit.mesure_units}" /></td>
			</tr>
			<tr>
		  ...
		  Описание
			...
			<tr>
				<td><input type="submit" value="Добавить новое изделие" /></td>

				<td><input type="submit" value="Изменить" /></td>

				<td><input type="button" class="button" value="Удалить"
					onclick="location.href='deleteunit/${unit.id}'" /></td>
			</tr>
		</table>
	</form:form>
